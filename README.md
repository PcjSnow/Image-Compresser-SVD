...
The SVD factorizes $A$ into three matrices: an orthogonal matrix U $x\in \mathbb{R}_{m\times m}$, a diagonal matrix $ \sigma \in \mathbb{R}_{m\times n}$ and another orthogonal matrix $V \in \mathbb{R}_{n\times n}$. The SVD allows as to compute A in terms of these matrices: $A = U\sigmaV^T$
